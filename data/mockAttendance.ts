...
export function getAttendanceSummary(
  ...
  const mealAttended = records.filter(
    r => r.type === 'meal' || r.type === 'both'
  ).length
  const mealAbsent = records.filter(
    r => r.type === 'general' && r.status === 'present'
  ).length
...
export function generateAttendanceReport(
  ...
  const studentSummaries = studentIds.map(id =>
    getAttendanceSummary(id, messId, startDate, endDate)
  )
